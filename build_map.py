...
<style>
  ...
  .fg-kommun    .filter-btn.active { background: #0d9488; border-color: #0d9488; }
...
// Kommun filter — the catchment is currently 6 kommuner. Match against the
// listing's `area` string ("…, Nacka kommun"). The genitive -s suffix is
// included in the regex where applicable (Stockholms, Danderyds, Sundbybergs).
const KOMMUN_FILTERS = [
  { key: 'Stockholm',  label: 'Stockholm',  re: /Stockholms kommun/  },
  { key: 'Danderyd',   label: 'Danderyd',   re: /Danderyds kommun/   },
  { key: 'Lidingö',    label: 'Lidingö',    re: /Lidingö kommun/     },
  { key: 'Nacka',      label: 'Nacka',      re: /Nacka kommun/       },
  { key: 'Solna',      label: 'Solna',      re: /Solna kommun/       },
  { key: 'Sundbyberg', label: 'Sundbyberg', re: /Sundbybergs kommun/ },
];
function kommunOf(d) {
  const area = d.area || '';
  for (const k of KOMMUN_FILTERS) if (k.re.test(area)) return k.key;
  return null;
}
...
const selectedKommuner  = new Set(savedFilters?.kommuner  ?? KOMMUN_FILTERS.map(k => k.key));

// Pre-compute counts.
const groupCounts = {}, statusCounts = {}, regionCounts = {}, featureCounts = {}, ownershipCounts = {}, kommunCounts = {};
sorted.forEach(d => {
  ...
  const km = kommunOf(d);
  if (km) kommunCounts[km] = (kommunCounts[km] || 0) + 1;
});
...
const kommunRow    = document.createElement('div'); kommunRow.className    = 'filter-group fg-kommun';
...
filtersEl.appendChild(kommunRow);
...
KOMMUN_FILTERS.forEach(k => {
  if (!kommunCounts[k.key]) return;
  kommunRow.appendChild(makeFilterButton(k.label, kommunCounts[k.key], selectedKommuner, k.key));
});
...
function passesKommunFilter(d) {
  const k = kommunOf(d);
  // Listings whose area doesn't match any of the 6 kommun patterns fall
  // through (no kommun-tag, no filtering) rather than getting silently
  // hidden — keeps ghost rows from older runs visible.
  return !k || selectedKommuner.has(k);
}
...
function updateVisibility() {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const filterOk = passesTypeFilter(d) && passesStatusFilter(d) && passesRegionFilter(d)
                     && passesFeatureFilter(d) && passesOwnershipFilter(d) && passesKommunFilter(d);
    if (marker) {
